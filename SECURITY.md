...
| ✅ **No data egress** | Nothing you enter is transmitted. No telemetry, analytics, phone-home, or tracking cookies. |
| ✅ **No secrets, no accounts** | No API key, no login, no server call is ever made. |
| ✅ **Reads only local data** | It fetches static JSON from the same origin — nothing else. |
