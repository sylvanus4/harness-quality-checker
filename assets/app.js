/* ─────────────────────────────────────────────────────────────────────────
   harness-quality-checker — DOM wiring (EN/KO).
   Scoring & generation live in compute.js (pure). This file fetches the rule
   set, reads the two textareas, and renders in the selected language. All
   user-supplied strings are escaped before insertion (no raw innerHTML of
   input). Chrome strings live in I18N; axis/signal/grade copy is bilingual in
   rules.json and picked by `lang`.
   ───────────────────────────────────────────────────────────────────────── */
(function () {
  ...
  let rules = null, examples = null, lang = "en";
  const mode = () => (document.querySelector('input[name="genmode"]:checked') || {}).value || "gaps";

  const I18N = {
    en: {
      sub: 'Paste your agent\'s <b>system prompt</b> and <b>tool schema</b>. A rule-based static analyzer scores the harness on five axes — <b>Loop · Verification · Tracing · Memory · Safety</b> — flags the exact gaps, and can <b>generate a hardened prompt</b> that satisfies all five. Like ESLint, but for agent harnesses. <b>No key · no server · no data leaves your browser.</b>',
      inHead: "Input", lblPrompt: "System prompt", hintPrompt: "the agent / MCP tool instructions",
      ...
      scoreHead: "Score", placeholder: "Paste a harness (or load an example) to see the five-axis breakdown.",
    ...
    ko: {
      sub: '에이전트의 <b>시스템 프롬프트</b>와 <b>툴 스키마</b>를 붙여넣으세요. 규칙 기반 정적 분석기가 하네스를 5축 — <b>루프 · 검증 · 추적 · 메모리 · 안전</b> — 으로 채점하고, 정확한 빈틈을 짚고, 다섯 축을 모두 만족하는 <b>강화된 프롬프트를 생성</b>합니다. ESLint의 에이전트 하네스판. <b>키 없음 · 서버 없음 · 데이터가 브라우저를 벗어나지 않음.</b>',
      inHead: "입력", lblPrompt: "시스템 프롬프트", hintPrompt: "에이전트 / MCP 툴 지시문",
      lblTools: "툴 / 함수 스키마", hintTools: "JSON — Anthropic 또는 OpenAI 형태 (선택)",
      exStrong: "강한 예시 불러오기", exWeak: "약한 예시 불러오기", clear: "지우기",
      scoreHead: "점수", placeholder: "하네스를 붙여넣으면(또는 예시를 불러오면) 5축 분석이 표시됩니다.",
      ...
      howScoreBody: "<ul><li>각 축은 가중 <b>신호</b>(정규식 문구 패턴 + 툴 JSON 구조 검사)를 가집니다. 해당 문구가 프롬프트/툴에 나타나거나 구조 검사를 통과하면 신호가 카운트됩니다.</li><li>축 점수 = <code>min(20, 매칭된 신호 가중치 합)</code>. 총점 = 5축 합 = <code>0–100</code>, 등급으로 매핑.</li><li><b>생성</b> 버튼은 같은 규칙으로 강화 프롬프트를 결정론적으로 조립합니다 — 각 조항이 분석기가 찾는 문구를 담고 있어 결과가 100점 근처로 재채점됩니다. 완성본이 아니라 다듬을 골격입니다.</li><li>규칙은 <code>data/rules.json</code>에 있고, 독립적인 파이썬 재구현이 모든 점수를 재도출해 CI에서 JS와 바이트 단위로 일치해야 합니다.</li><li><b>한계:</b> 이것은 <em>린트</em>이지 증명이 아닙니다. 문구와 구조를 읽으므로, 특이하게 표현된 개념을 놓치거나 실제로 배선되지 않은 문구에 점수를 줄 수 있습니다. 낮은 점수는 \"이 관심사를 명시하라\", 높은 점수는 \"기술은 했다\"로 읽되 런타임 보장으로 여기지 마세요.</li></ul>",
  ...
  function loadExample(id) {
    if (id === "clear") { promptEl.value = ""; toolsEl.value = ""; genOut.value = ""; $("btnCopy").disabled = true; render(); return; }
    if (!examples) return;
    const c = examples.cases.find(function (x) { return x.id === id; });
    if (!c) return;
    promptEl.value = c.prompt; toolsEl.value = c.tool; render();
  }
